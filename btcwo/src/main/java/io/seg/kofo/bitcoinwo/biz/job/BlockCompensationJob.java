...
public class BlockCompensationJob  {
    ...
    @Autowired
    CompensationHeightService compensationHeightService;
    @Autowired
    WatchOnlyProperties watchOnlyProperties;
    @Autowired
    FullNodeCache fullNodeCache;
    @Value("${msg.generateConcurrentThread:5}")
...
//    @PostConstruct
    ...
    public List<CompensationHeightPo> fetchData() {
        ...
        //表中只有一条数据
        CompensationHeightPo syncHeightPo = compensationHeightService.selectOne(CompensationHeightPo.builder().build());
    ...
    public void processData(CompensationHeightPo preSyncHeight) {
        ...
        if(Objects.nonNull(originBlockMsgPo)){
            //如果该高度已存在 则不需要进行rpc查询
            log.info("Compensation exist block: {} {}, skip", originBlockMsgPo.getHeight(), originBlockMsgPo.getHash());

            compensationHeightService.updateSyncHeight(MsgQueuePo.builder()
                    .height(originBlockMsgPo.getHeight())
                    .blockHash(originBlockMsgPo.getHash())
                    .build());
            return ;
        }
        BtcBlockHeightPo btcBlockHeightPo = blockHeightService.selectOne(BtcBlockHeightPo.builder().build());
